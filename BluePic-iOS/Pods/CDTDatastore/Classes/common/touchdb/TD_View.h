...
/** A "map" function called when a document is to be added to a view.
    ...
    @param emit  A block to be called to add a key/value pair to the view. Your block can call it
...
/** Compiles a view (using the registered TDViewCompiler) from the properties found in a
 * CouchDB-style design document. */
...
/** Queries the view. Does NOT first update the index.
    ...
    @return  An array of result rows -- each is a dictionary with "key" and "value" keys, and
   possibly "id" and "doc". */
