...
/** Total length of the stream.
    This is just computed by adding the values passed to -addStream:length:, and the lengths of the
   NSData objects and files added.
    If -addStream: has been called (the version without length:) the length is unknown and will be
   returned as -1.
    (Many clients won't care about the length, but TDMultipartUploader does.) */
@property (readonly) SInt64 length;

/** Returns an input stream; reading from this will return the contents of all added streams in
   sequence.
...
/** Associates an output stream; the data from all of the added streams will be written to the
