...
/** Escapes a document or revision ID for use in a URL.
    This does the usual %-escaping, but makes sure that '/' is escaped in case the ID appears in the
   path portion of the URL, and that '&' is escaped in case the ID appears in a query value. */
...
/** Wraps a string in double-quotes and prepends backslashes to any existing double-quote or
 * backslash characters in it. */
NSString* TDQuoteString(NSString* param);

/** Undoes effect of TDQuoteString, i.e. removes backslash escapes and any surrounding
   double-quotes.
    If the string has no surrounding double-quotes it will be returned as-is. */
NSString* TDUnquoteString(NSString* param);

/** Returns YES if this error appears to be due to the computer being offline or the remote host
 * being unreachable. */
...
/** Returns the input URL without the query string or fragment identifier, just ending with the
 * path. */
